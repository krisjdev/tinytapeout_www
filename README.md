...
## Adding a new shuttle page

Run `hugo new chips/<SHUTTLE_ID>` where `<SHUTTLE_ID>` is a valid ID that exists within the Tiny Tapeout index API. For
example: `hugo new chips/ttsky26b`.

This may take a moment as it fetches information from the API - this is expected. You may add content to the page,
but this will prevent it from being dynamically generated by the server. Simply commit the new shuttle page as-is and it
should be automatically populated with info from the index API.
